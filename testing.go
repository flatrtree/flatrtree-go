...
var testDegrees = []int{5, DefaultDegree, 15}

var testBuilders = map[string]func() Builder{
	"Hilbert": func() Builder { return NewHilbertBuilder() },
	"OMT":     func() Builder { return NewOMTBuilder() },
}

type rtreeTestCase struct {
	name   string
	index  *RTree
	items  []float64
	count  int
...
func createTestCases(t *testing.T) []rtreeTestCase {
	var result []rtreeTestCase
	for builderName, newBuilder := range testBuilders {
		for _, degree := range testDegrees {
			for _, count := range []int{0, 1, degree, 100} {
				name := fmt.Sprintf("%v/deg=%d/count=%d", builderName, degree, count)
				index, items := createIndex(t, newBuilder, count, degree)
				result = append(result, rtreeTestCase{name, index, items, count, degree})
			}
		}
	}
	return result
...
func createIndex(t *testing.T, newBuilder func() Builder, count int, degree int) (*RTree, []float64) {
	var testBoxes = []float64{
	...
	items := testBoxes[:count*4]

	builder := newBuilder()
	for i := 0; i < len(items)/4; i++ {
		builder.Add(int64(i), items[i*4], items[i*4+1], items[i*4+2], items[i*4+3])
	}

	index, err := builder.Finish(degree)
	require.Nil(t, err)
